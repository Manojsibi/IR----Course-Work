...
def index_docs(docs, *fields):
    index: defaultdict[Any, defaultdict[Any, Counter]] = defaultdict(lambda: defaultdict(Counter))
...
def createjsondata(data, doc_id):
    return {
        ...
        'paperUrl': data[doc_id]['PaperUrl'],
        'publishedYear': data[doc_id]['PublishedDate'],
        'authors': data[doc_id]['Authors'],
        'description': data[doc_id]['description'],
        'fingerprint' : data[doc_id]['fingerprint']
    }
